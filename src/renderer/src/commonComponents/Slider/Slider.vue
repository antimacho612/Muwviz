...
const updateCurrentValue = (newValue: number, emit = true) => {
  current.value = parseFloat(Math.min(Math.max(newValue, props.min), props.max).toFixed(10));
...
const onKeyDown = (event: KeyboardEvent) => {
  switch (event.code) {
    case 'ArrowUp':
    case 'ArrowRight':
      incrementCurrentValue(props.step || 1);
      event.preventDefault();
      break;
    case 'ArrowDown':
    case 'ArrowLeft':
      decrementCurrentValue(props.step || 1);
      event.preventDefault();
      break;
    case 'PageUp':
      incrementCurrentValue(props.step || 10);
      event.preventDefault();
      break;
    case 'PageDown':
      decrementCurrentValue(props.step || 10);
      event.preventDefault();
      break;
    case 'End':
